			...
			if boardToSimulate.point_check_game_end_gomoku(move,opponentSimulationColor,5) == True:
				...
				if(winDirection == 'vertical'):
					...
					if v1 == True or v2 == True:
						if move not in OpenFourMoves:
							OpenFourMoves.append(move)
				if(winDirection == 'diagonal'):
					#How to check empty point to move's left and right.
					#if there is a single empty point. we add to list.
					#if no empty point we dont add.
					moveNeighbours = boardToSimulate._diag_neighbors(move)
					d1 = boardToSimulate.is_legal_gomoku(moveNeighbours[0],playerSimulationColor)
					d2 = boardToSimulate.is_legal_gomoku(moveNeighbours[3],playerSimulationColor)
					if d1 == True or d2 == True:
						if move not in OpenFourMoves:
							OpenFourMoves.append(move)				
				if(winDirection == 'antidiagonal'):
					#How to check empty point to move's left and right.
					#if there is a single empty point. we add to list.
					#if no empty point we dont add.
					moveNeighbours = boardToSimulate._diag_neighbors(move)
					d3 = boardToSimulate.is_legal_gomoku(moveNeighbours[1],playerSimulationColor)
					d4 = boardToSimulate.is_legal_gomoku(moveNeighbours[2],playerSimulationColor)
				...
				winDirection = boardToSimulate.point_check_game_end_gomoku_a3(move,opponentSimulationColor,4)[1]
				if(winDirection == 'horizontal'):
					#How to check empty point to move's left and right.
					#if there is a single empty point. we add to list.
					#if no empty point we dont add.
					moveNeighbours = boardToSimulate._neighbors(move)
					h1 = boardToSimulate.is_legal_gomoku(moveNeighbours[0],opponentSimulationColor)
					h2 = boardToSimulate.is_legal_gomoku(moveNeighbours[1],opponentSimulationColor)
					if h1 == True or h2 == True:
				...
				if(winDirection == 'vertical'):
					...
					#if no empty point we dont add.
					moveNeighbours = boardToSimulate._neighbors(move)
					v1 = boardToSimulate.is_legal_gomoku(moveNeighbours[2],opponentSimulationColor)
					v2 = boardToSimulate.is_legal_gomoku(moveNeighbours[3],opponentSimulationColor)
					if v1 == True or v2 == True:
						if move not in BlockOpenFourMoves:
							BlockOpenFourMoves.append(move)
				if(winDirection == 'diagonal'):
					...
					#if no empty point we dont add.
					moveNeighbours = boardToSimulate._diag_neighbors(move)
					d1 = boardToSimulate.is_legal_gomoku(moveNeighbours[0],opponentSimulationColor)
					d2 = boardToSimulate.is_legal_gomoku(moveNeighbours[3],opponentSimulationColor)
					if d1 == True or d2 == True:
						if move not in BlockOpenFourMoves:
							BlockOpenFourMoves.append(move)	
				if(winDirection == 'antidiagonal'):
					...
					#if no empty point we dont add.
					moveNeighbours = boardToSimulate._diag_neighbors(move)
					d3 = boardToSimulate.is_legal_gomoku(moveNeighbours[1],opponentSimulationColor)
					d4 = boardToSimulate.is_legal_gomoku(moveNeighbours[2],opponentSimulationColor)
					if d3 == True or d4 == True:
						if move not in BlockOpenFourMoves:
							BlockOpenFourMoves.append(move)																					
				if move not in BlockOpenFourMoves:
